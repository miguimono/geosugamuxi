...
module.exports = {
  ...
  async getExperienceBasic(req, res) {
    let experience = await model_experience.findAll({
      include: [
        {
          model: model_experience_heritage_site,
  ...
  async getExperienceByName(req, res) {
    ...
    for (var i = 0; i < experience.length; i++) {
      ...
      for (let j = 0; j < experience[i].experience_heritage_sites.length; j++) {
        heritage_sites.push({
          id_heritage_site:
            experience[i].experience_heritage_sites[j].id_heritage_site,
          name_heritage_site:
  ...
  async getExperienceById(req, res) {
    let experience = await model_experience.findAll({
      ...
      where: {
        id_experience: req.params.id_experience
      }
    });
    var data = [];
    var heritage_sites = [];
  ...
  async getExperienceSize(req, res) {
    let experience = await model_experience.findAll({});
    res.json(experience.length);
  }
};
